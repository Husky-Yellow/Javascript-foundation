...
namespace HAOLAI.Models.COM
{
    ...
    public class Pic_Class
    {
        ...
        /// 自动修改图片大小
        /// </summary>
        /// <param name="urlName">图片绝对路径+名称</param>
        /// <param name="size">压缩力度1-100</param>
        /// <returns></returns>
        public static bool AutoChangeImgSize(string urlName, int size)
        {
            //第一步：生成出一张压缩好的图片（原名称+"a"）
            bool atrm = new Pic_Class().GetPicThumbnail(urlName, urlName + "a", size);
            //第二步：删除原图
            if (atrm)
            {
                if (File.Exists(urlName))
                {
                    try
                    {
                        FileInfo fi = new FileInfo(urlName);
                        if (fi.Attributes.ToString().IndexOf("ReadOnly") != -1)
                            fi.Attributes = FileAttributes.Normal;
                        File.Delete(urlName);
                        //第三步：将新生成的图片改成旧图片名
                        File.Move(urlName + "a", urlName);
                        return true;
                    }
                    catch (Exception)
                    {
        ...
        #endregion
        #region 创建两个图片（大图和缩略图）
        /// <summary>
        /// 创建两个图片（大图和缩略图）
        /// </summary>
        /// <param name="originalImagePath">保存路径</param>
        /// <param name="USofBigAndSmall">大图与缩略图尺寸：[0]宽,[1]高</param>
        /// <param name="flag">压缩力度</param>
        /// <returns></returns>
        public Boolean MakingImgThree(string ImageSavePath, string Name, string[] USofBigAndSmall, int flag)
        {
            Pic_Class.MakeThumbnail(ImageSavePath + Name, ImageSavePath + "[little]" + Name, int.Parse(USofBigAndSmall[0]), int.Parse(USofBigAndSmall[1]), "Cut");
            if (!GetPicThumbnail(ImageSavePath + Name, ImageSavePath + "[big]" + Name, flag)) { return false; };
            return true;
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public static string UploadPicThree()
        {
